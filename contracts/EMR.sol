...
modifier higherThanZero(uint256 _uint) {
 ...
 function giveAccessToIns(address insurance) public {
  ...
  function revokeAccessFromDoctor(address doctor, uint index) public {
   ...
   function revokeAccessFromIns(address insurance, uint index) public {
    require(insurancePermissions[insurance][index] == msg.sender, 'You can only revoke access to your own documents.');
    delete insurancePermissions[insurance][index];
  }
